...
def analyze_single_recording(recording):
    ...
    for frame_index, frame in enumerate(recording.sensor_readings):
        timestamp = recording.start_time + pd.Timedelta(seconds=frame_index * 0.1)  # Assuming original data is at 10 Hz
    ...
    average_load_per_hold = calculate_average_load_per_hold(active_df)
    load_distribution = calculate_load_distribution(active_df)
    peak_load_rate = calculate_peak_load_rate(active_df)
    hold_engagement_sequence = extract_hold_engagement_sequence(active_df)
    
    # Generate plots
    load_time_series_plot = generate_load_time_series_plot(df, recording.id)
    hold_load_time_series_plots = generate_hold_load_time_series_plots(df, recording.id)
...
def extract_hold_engagement_sequence(df):
    # Determine when each hold is engaged
...
def generate_hold_load_time_series_plots(df, recording_id):
    hold_plots = {}
    holds = df['hold_id'].unique()
    for hold in holds:
        df_hold = df[df['hold_id'] == hold]
        df_hold_time = df_hold.groupby('timestamp')['force_magnitude'].sum().reset_index()
        fig = px.line(df_hold_time, x='timestamp', y='force_magnitude', title=f'Load Over Time for Hold {hold} in Recording {recording_id}')
